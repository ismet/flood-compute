...
| `data/tables/mgm_plv_2020.json` | MGM 2020 tablosu — **yalnız 14 plüviyograf (PLV) oranı** için kullanılır. `/api/mgm-stations` P24 sütunlarını bilerek döndürmez; P2–P100 ve OEY elle veya Excel'den girilir. `tools/extract_mgm_plv.py` ile üretilir. |
...
| `data/mgm/` | `mgm-istasyonlari.json` — MGM'den indirilen 81 il / 1913 istasyonluk kanonik sicil; `YagisSensor=1` olan 370 istasyon varsayılan Thiessen kümesidir. `tools/mgm_veritabani_olustur.py` dosyayı değiştirmeden doğrular. Eski ölçüm SQLite'ı yalnız denetim/geri dönüş için `archive/` altındadır; çalışma zamanında açılmaz. Ayrıntı: `data/mgm/MIGRATION-2026-08-31.md`. |
...
3. **Yağış — Thiessen + Yağış birleşik** — Üstte **Thiessen**: varsayılan küme kanonik MGM sicilinde güncel `YagisSensor=1` işareti bulunan **370 istasyondur** (`data/mgm/mgm-istasyonlari.json`). Bu işaret tarihsel seri yeterliliği anlamına gelmez. Voronoi hücreleri havzaya kesilerek alan ağırlıkları bulunur; yakın sicil istasyonları elle eklenebilir ve kendi KMZ/KML kümeniz yüklenebilir. Altta **Yağış**: P2–P100 ve OEY değerleri elle girilir veya Excel'den yapıştırılır; değerler ad yerine kararlı istasyon kimliğiyle saklanır. Eski projelerin gömülü, ad anahtarlı manuel değerleri yüklemede korunur. DPLV zaman dağılımı 236 MGM PLV satırından elle seçilebilir; yalnız kanonik sicille tek anlamlı eşleşen 215 istasyon otomatik en-yakın seçimindedir (`POST /api/plv-en-yakin`).
