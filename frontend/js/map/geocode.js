...
(function geocodeSearch() {
  ...
  function render() {
    if (!items.length) {
      box.innerHTML = `<div class="geo-empty">Sonuç yok</div>`;
    ...
    box.innerHTML = items
      ...
      .join("");
    box.classList.remove("hidden");
    box.querySelectorAll(".geo-item").forEach((el) => (el.onclick = () => choose(+el.dataset.i)));
  }
  function choose(i) {
    const it = items[i];
    if (!it) return;
  ...
  document.addEventListener("click", (e) => {
    if (!$("mapSearch").contains(e.target)) hide();
  });
})();
