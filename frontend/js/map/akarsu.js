...
async function akarsuYukle() {
  ...
  if (map.getZoom() < AKARSU_MIN_ZOOM) {
    ...
    $("akarsuInfo").textContent = `yakınlaştırın (z≥${AKARSU_MIN_ZOOM}) — bu ölçekte tüm ülke yüklenemez`;
    return;
  }
  ...
  const bati = sy(b.getWest(), -180, 179.999),
    dogu = sy(b.getEast(), -179.999, 180);
  const guney = sy(b.getSouth(), -90, 89.999),
    kuzey = sy(b.getNorth(), -89.999, 90);
  ...
  try {
    ...
    $("akarsuInfo").textContent = `Hata: ${e.name}: ${e.message}${ag ? " (istek sunucuya ulaşmadı)" : ""} — ${url}`;
  }
}

$("akarsuAc").onchange = () => {
  if ($("akarsuAc").checked) {
    layers.akarsu.addTo(map);
    akarsuYukle();
  } else {
    layers.akarsu.remove();
    layers.akarsu.clearLayers();
    $("akarsuInfo").textContent = "";
  }
};
$("akarsuOlcek").onchange = () => {
  if ($("akarsuAc").checked) akarsuYukle();
};
...
(async function akarsuDurum() {
  try {
    ...
    if (!b.var) {
      ...
      $("akarsuInfo").textContent = b.olcekler
        .map((o) => `1/${o.olcek}.000: ${o.kol.toLocaleString("tr")}`)
        .join(" · ");
    }
  } catch (e) {
    /* uç yoksa sessiz geç */
