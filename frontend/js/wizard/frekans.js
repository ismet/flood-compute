...
function agiBolgeselDegis(kod, ac) {
  if (ac) S.agiBolgesel.add(kod);
  else S.agiBolgesel.delete(kod);
  agiIsaretle();
}
...
function agiListele(ist) {
  ...
  const bas =
    "<tr><th title='bölgesel analize dahil et'>BTFA</th><th>Kod</th><th>Ad</th>" +
    "<th>Kurum</th><th>Yıl</th><th>Aralık</th><th>A (km²)</th></tr>";
  let h =
    '<div class="rain-tools"><button id="agiHepsi" class="small-btn">' +
    "Tümünü BTFA'ya ekle</button>" +
    '<button id="agiHicbiri" class="small-btn">Seçimi temizle</button></div>';
  if (icinde.length)
    h +=
      `<p class="small"><b>Havza içinde (${icinde.length})</b></p>` +
      `<table class="tbl small">${bas}${icinde.map(sat).join("")}</table>`;
  if (disinda.length)
    h +=
      `<p class="small"><b>Çevrede (${disinda.length})</b></p>` +
  ...
  $("agiListe")
    .querySelectorAll(".agi-bol")
    .forEach((cb) => {
      cb.onclick = (e) => {
        e.stopPropagation();
        agiBolgeselDegis(cb.dataset.kod, cb.checked);
      };
    });
  $("agiListe")
    .querySelectorAll(".agi-sat")
    .forEach((tr) => {
      tr.onclick = () => {
        const s = ist.find((x) => x.kod === tr.dataset.kod);
        if (s) {
          agiSec(s);
          map.setView([s.enlem, s.boylam], Math.max(map.getZoom(), 11));
        }
      };
    });
  const hepsi = $("agiHepsi"),
    hicbiri = $("agiHicbiri");
  if (hepsi)
    hepsi.onclick = () => {
      ist.forEach((s) => {
        if (s.yagis_alani) S.agiBolgesel.add(s.kod);
      });
      agiListele(ist);
    };
  if (hicbiri)
...
async function agiYukle() {
  ...
  try {
    ...
    setStatus(
      "tfaStatus",
      `${r.istasyonlar.length} istasyon bulundu — ` + "haritadan ya da listeden birini seçin.",
      "ok",
    );
  } catch (e) {
...
async function agiKatmanAc() {
  try {
    ...
    if (!b.var) {
      ...
      $("agiInfo").textContent = "veri yok — tools/agi_veritabani_olustur.py ile üretin";
      return;
    }
...
function tfaAykiriBlok(o) {
  ...
  const y = a.yuksek || [],
    d = a.dusuk || [];
  let h =
    `<p class="small"><b>Aykırı değer testi (Grubbs-Beck, Bulletin 17B)</b> — ` +
    `n=${a.n}, K<sub>n</sub>=${fmt(a.kn, 3)}, ` +
    `üst sınır ${fmt(a.ust_sinir, 1)} · alt sınır ${fmt(a.alt_sinir, 1)} m³/s</p>`;
  if (!y.length && !d.length) return h + `<p class="small">Aykırı değer yok — seri sınırlar içinde.</p>`;
  h +=
    `<p class="small">` +
  ...
  if (!ay)
    return (
      h +
      `<p class="small">Aykırısız sonucu görmek için ` +
      `"aykırıları çıkarıp karşılaştır" kutusunu işaretleyin.</p>`
    );

  const T = o.tekerrur,
    bas = (t) => `<th style="text-align:right">${t}</th>`;
  const sag = (v) => `<td style="text-align:right">${fmt(v, 1)}</td>`;
  h +=
    `<p class="small"><b>Aykırılı ↔ aykırısız karşılaştırma (kabul edilen dağılım)</b></p>` +
    `<table class="tbl small"><tr><th>Durum</th><th>n</th><th>Dağılım</th>` +
    T.map(bas).join("") +
    `</tr>` +
    `<tr><td>aykırılı</td><td>${o.parametreler.yil_sayisi}</td>` +
    `<td>${o.kabul_edilen_adi}</td>` +
    (o.kabul_edilen_q || []).map(sag).join("") +
    `</tr>` +
    `<tr><td>aykırısız</td><td>${ay.parametreler.yil_sayisi}</td>` +
    `<td>${ay.kabul_edilen_adi}</td>` +
    (ay.kabul_edilen_q || []).map(sag).join("") +
    `</tr>` +
    `<tr class="sel"><td colspan="3"><b>fark</b></td>` +
    T.map((t, i) => {
      const a1 = (o.kabul_edilen_q || [])[i],
        a2 = (ay.kabul_edilen_q || [])[i];
...
function tfaCiz(o) {
  ...
  h +=
    '<p class="small"><b>Tekerrür debileri (m³/s)</b></p><table class="tbl small">' +
    "<tr><th>Dağılım</th>" +
    T.map((t) => bas(t)).join("") +
    "<th>Kabul</th></tr>";
  o.debiler.forEach((d) => {
    h +=
      `<tr${d.kabul_edilen ? ' style="font-weight:600"' : ""}><td>${d.dagilim}</td>` +
  ...
  h +=
    '<p class="small"><b>İstatistik parametreler</b></p><table class="tbl small">' +
  ...
  h +=
    '<p class="small"><b>Simirnov-Kolmogorov testi</b></p><table class="tbl small">' +
    "<tr><th>Dağılım</th>" +
    bas("Teorik Pi") +
    bas("Amprik Pi") +
    bas("Dmaks") +
    bas("Gözlem") +
    o.ks_anlamlilik.map((a) => bas(Math.round(a * 100) + "%")).join("") +
    "</tr>";
  o.ks_testi.forEach((s) => {
    ...
    h +=
      ...
      o.ks_anlamlilik.map((a) => `<td style="text-align:center">${s.kabul[a] ? "Kabul" : "Red"}</td>`).join("") +
      "</tr>";
  });
  h += "</table>";
  ...
  btfaHomChart = new Chart($("btfaHomojenGrafik"), {
    ...
    data: {
      datasets: [
        {
          label: "üst sınır (%95)",
          data: hm.zarf.map((z) => ({ x: z.n, y: z.t_ust })),
          borderColor: "#1565c0",
          borderWidth: 1.5,
          borderDash: [6, 3],
          pointRadius: 0,
        ...
        {
          label: "alt sınır (%95)",
          data: hm.zarf.map((z) => ({ x: z.n, y: z.t_alt })),
          borderColor: "#1565c0",
          borderWidth: 1.5,
          borderDash: [6, 3],
          pointRadius: 0,
        ...
        {
          label: `T = ${hm.t_merkez} yıl`,
          data: hm.zarf.map((z) => ({ x: z.n, y: hm.t_merkez })),
          borderColor: "#9e9e9e",
          borderWidth: 1,
          pointRadius: 0,
        },
...
function btfaKarsilastir(o) {
  const a = o.aykirisiz;
  if (!a) return o.aykirisiz_hata ? `<p class="small"><b>${o.aykirisiz_hata}</b></p>` : "";
  ...
  return (
    `<p class="small"><b>Aykırılar çıkarılınca</b> — çıkarılan: ` +
    `${a.cikarilan.join(", ")} (${o.kullanilan_sayisi} → ${a.kullanilan_sayisi} istasyon). ` +
    (a.homojenlik.homojen ? "Kalan bölge <b>homojen</b>." : `Hâlâ aykırı var: ${a.homojenlik.aykiri.join(", ")}.`) +
    '</p><table class="tbl small"><tr><th>Durum</th>' +
    T.map((t) => `<th style="text-align:right">${t}</th>`).join("") +
    "</tr>" +
    "<tr><td>Tüm istasyonlar</td>" +
    o.btfa.q.map(sag).join("") +
    "</tr>" +
    "<tr><td>Aykırısız</td>" +
    a.btfa.q.map(sag).join("") +
    "</tr>" +
    "<tr><td>Fark</td>" +
    fark
      .map(
        (v) =>
          `<td style="text-align:right;color:${v == null ? "#666" : Math.abs(v) > 10 ? "#c62828" : "#666"}">` +
          `${v == null ? "—" : (v > 0 ? "+" : "") + fmt(v, 1) + "%"}</td>`,
      )
      .join("") +
    "</tr></table>" +
...
function btfaCiz(o) {
  ...
  let h =
    '<p class="small"><b>Bölgesel analizde kullanılan AGİ\'ler</b> ' +
  ...
  h +=
    '<p class="small"><b>Bölgesel büyüme eğrisi</b> (Q<sub>T</sub>/Q<sub>2</sub> ortalaması)' +
    '</p><table class="tbl small"><tr><th>T (yıl)</th>' +
    T.map((t) => `<th style="text-align:right">${t}</th>`).join("") +
    "</tr><tr><td>oran</td>" +
    o.buyume_egrisi.map((v) => sag(v, 4)).join("") +
    "</tr></table>";

  const b = o.bagintis,
    rs = b.regresyon_serbest,
    r1 = b.regresyon_a1;
  h +=
    '<p class="small"><b>İndeks debi bağıntısı</b> — kullanılan: ' +
  ...
  const bt = o.btfa;
  h +=
    `<p class="small"><b>Havza taşkın debileri</b> — A = ${fmt(o.alan_km2, 2)} km², ` +
    `Q<sub>2</sub> = ${fmt(o.q2_indeks, 2)} m³/s</p><table class="tbl small">` +
    "<tr><th>Yöntem</th>" +
    bt.tekerrur
      .map((t, i) => `<th style="text-align:right">${t}${i >= bt.ekstrapole_baslangic ? "*" : ""}</th>`)
      .join("") +
    "</tr><tr><td><b>BTFA</b></td>" +
    bt.q.map((v) => sag(v)).join("") +
    "</tr>";
  if (o.ntfa_transfer) {
    const t2 = o.ntfa_transfer;
    h +=
      `<tr><td>NTFA aktarım<br><span class="small">${t2.kod}, ` +
      `(A/${fmt(t2.kaynak_alan, 1)})<sup>${fmt(t2.us, 3)}</sup></span></td>` +
      t2.q.map((v) => sag(v)).join("") +
      "</tr>";
  }
  h +=
    "</table><p class='small'>* Q500 ve üzeri, Q10–Q100'den ekstrapole edilmiştir " +
...
$("btnBtfa").onclick = async () => {
  ...
  if (!alan)
    return setStatus("btfaStatus", "Havza alanı (km²) gerekli — 1. adımda havzayı çıkarın ya da elle yazın.", "err");
  ...
  try {
    ...
    setStatus(
      "btfaStatus",
      `${o.kullanilan_sayisi} istasyon kullanıldı` +
        (at ? `, ${at} tanesi dışarıda kaldı` : "") +
        ` — Q100 = ${fmt(o.btfa.q[5], 1)} m³/s` +
        (o.aykirisiz
          ? ` · aykırısız (${o.aykirisiz.kullanilan_sayisi} istasyon): ` + `${fmt(o.aykirisiz.btfa.q[5], 1)} m³/s`
          : o.homojenlik && o.homojenlik.homojen
            ? " · bölge homojen"
            : "") +
        ".",
      "ok",
