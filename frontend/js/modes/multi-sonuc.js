...
/* === extracted to core/constants.js M_LABEL === */ function _envPeak(res, rp) {
  ...
  ["2", "4", "6", "8", "12", "18", "24"].forEach((d) => {
    const v = res.kabulet[d] && res.kabulet[d][rp];
...
function profilTani(elevations, L_km) {
  ...
  if (enBuyukPay > 0.35)
    uyarilar.push(
      `Eğimi tek segment belirliyor: <b>H${enBuyukIdx}–H${enBuyukIdx + 1}</b> ` +
  ...
  if (oran > 5)
    uyarilar.push(
      `Harmonik eğim, ortalama eğimin <b>${oran.toFixed(1)} katı</b> altında ` +
        `(harmonik %${(S_harm * 100).toFixed(3)} — ortalama %${(S_ort * 100).toFixed(3)}); ` +
        `profilde düz bölümler baskın.`,
    );
  if (S_harm < 0.0005)
    uyarilar.push(
      `Harmonik eğim çok düşük (%${(S_harm * 100).toFixed(4)}) — ` +
        `T<sub>c</sub> gerçekçi olmayacak kadar büyük çıkar.`,
    );
  if (dhTop < L_km * 0.5)
    uyarilar.push(
      `Toplam kot farkı ${dhTop.toFixed(1)} m, ${L_km.toFixed(1)} km uzunluk için ` +
        `çok az (ortalama eğim %${(S_ort * 100).toFixed(3)}). DEM profili kusurlu olabilir.`,
    );
  const kucukler = dh.filter((d) => d <= 0.5).length;
  if (kucukler) uyarilar.push(`${kucukler} segmentte kot artışı ≤0,5 m — düz/yamalı profil.`);
...
function openParams() {
  ...
  h +=
    `<h3 class="res">Kot Profili (outlet → memba, 11 nokta, m)</h3><table class="tbl">
    <tr><th>Havza</th>` +
    Array.from({ length: 11 }, (_, i) => `<th>H${i}</th>`).join("") +
    `<th>Δh top.</th></tr>`;
  satirlar.forEach((r) => {
  ...
  h +=
    `<h3 class="res">Segment Eğim Payları (harmonik eğime katkı)</h3><table class="tbl">
    <tr><th>Havza</th>` +
    Array.from({ length: 10 }, (_, i) => `<th>H${i}–H${i + 1}</th>`).join("") +
    `</tr>`;
  taniLar.forEach((x) => {
    if (!x.t) {
      h += `<tr><td>${x.ad}</td><td colspan="10">—</td></tr>`;
      return;
    }
    h +=
      `<tr><td>${x.ad}</td>` +
      x.t.paylar.map((p) => `<td${p > 0.35 ? ' class="max"' : ""}>%${(p * 100).toFixed(0)}</td>`).join("") +
      `</tr>`;
  });
  ...
  if (uyarili.length) {
    ...
    uyarili.forEach((x) => {
      h +=
        `<div class="small err" style="margin-bottom:6px"><b>${x.ad}</b><ul style="margin-left:18px">` +
        x.t.uyarilar.map((u) => `<li>${u}</li>`).join("") +
        `</ul></div>`;
    });
  ...
  satirlar.forEach((r) => {
    ...
    h +=
      `<tr><td>${r.ad}</td><td style="text-align:left">` +
  ...
  h +=
    `<h3 class="res">Ağırlıklı 24 Saatlik Yağış (mm)</h3><table class="tbl">
    <tr><th>Havza</th>` +
    RPL.map((t) => `<th>P${t}</th>`).join("") +
    `<th>OEY</th></tr>`;
  satirlar.forEach((r) => {
    const g = r.c.girdi;
    h +=
      `<tr><td>${r.ad}</td>` +
      RPL.map((t) => `<td>${fmt((g.P24 || {})[t], 1)}</td>`).join("") +
  ...
  satirlar.forEach((r, k) => {
    ...
    ds.push({
      ...
      data: noktalar,
      borderColor: RENK[k % RENK.length],
      ...
      pointBackgroundColor: noktalar.map((_, i) =>
        t && t.enBuyukPay > 0.35 && (i === t.enBuyukIdx || i === t.enBuyukIdx + 1) ? "#c73e3a" : RENK[k % RENK.length],
      ),
    });
  });
...
function renderMultiResults() {
  ...
  const _tani = membaC
    .map((x, i) => ({ ad: "Memba " + (i + 1), t: profilTani(x.girdi.elevations, x.girdi.L_km) }))
    .concat([{ ad: "Ara havza", t: profilTani(araC.girdi.elevations, araC.girdi.L_km) }])
    .filter((x) => x.t && x.t.uyarilar.length);
  if (_tani.length)
    h += `<div class="small err">⚠ Kot profili şüpheli: ${_tani.map((x) => x.ad).join(", ")} — T<sub>c</sub> ve öteleme süresi olduğundan büyük çıkabilir. Ayrıntı için <b>📐 Fiziksel Parametreler</b>.</div>`;

  // 2) mansap pikleri — yöntem × tekerrür
  if (rezVar) {
    const rl = rt.yontemler,
      rs = rt.yontemler_rezervuarsiz;
    ...
    methods.forEach((m) => {
      const a = rs[m] && rs[m].pikler["100"],
        b = rl[m] && rl[m].pikler["100"];
  ...
  h +=
    `<h3 class="res">Mansap Taşkın Pikleri (öteleme=${fmt(md.ara.Tc_saat, 2)} sa, m³/s)</h3>
    <table class="tbl"><tr><th>Yöntem</th>` +
    MRP.map((rp) => `<th>Q${rp}</th>`).join("") +
    `</tr>`;
  methods.forEach((m) => {
    const y = Y[m];
    if (!y) return;
    ...
    h +=
      `<tr><td>${M_LABEL[m]}${syn}</td>` +
      MRP.map((rp) => `<td>${y.pikler[rp] == null ? "—" : fmt(y.pikler[rp], 1)}</td>`).join("") +
      `</tr>`;
  });
  ...
  if (comp)
    h += `<div class="small">${M_LABEL[dm]} Q100 bileşen: ara ${fmt(comp.ara_pik, 1)} +
  ...
  if (rezVar) {
    ...
    if (cl)
      cl.onclick = async () => {
        S.multiRes = {};
        const m = await import("./multi.js");
        await m.reRouteMulti();
      };
  }
  $("btnMcmp").onclick = openMcmp;
  $("btnResMulti").onclick = async () => {
    const m = await import("./rezervuar.js");
    m.openReservoir();
  };
...
function openMcmp() {
  ...
  $("mcmpMethod").onchange = () => {
    mcmpState.method = $("mcmpMethod").value;
    renderMcmp();
  };
  $("mcmpRP").onchange = () => {
    mcmpState.rp = $("mcmpRP").value;
    renderMcmp();
  };
  document.querySelectorAll(".mcmp-tab").forEach(
    (b) =>
      (b.onclick = () => {
        document.querySelectorAll(".mcmp-tab").forEach((x) => x.classList.remove("active"));
        b.classList.add("active");
        mcmpState.tab = b.dataset.tab;
        renderMcmp();
      }),
  );
...
function renderMcmp() {
  ...
  $("mcmpRP").innerHTML = opts
    .map((rp) => `<option value="${rp}" ${rp === mcmpState.rp ? "selected" : ""}>Q${rp}</option>`)
    .join("");
...
function _mkChart(datasets, title, parsing) {
  ...
  mcmpChart = new Chart($("mcmpChart"), {
    ...
    options: {
      animation: false,
      maintainAspectRatio: false,
      parsing: parsing === "bar" ? undefined : false,
...
function renderMcmpBilesen() {
  const { rt, md } = S.multiSonuc,
    m = mcmpState.method,
    rp = mcmpState.rp;
  const y = rt.yontemler[m],
    b = y.bilesenler[rp],
    dt = y.dt,
    shift = y.shift_adim;
  const ds = [];
  ds.push({
    label: "Ara havza",
    data: b.ara_h.map((v, i) => ({ x: i * dt, y: v })),
    borderColor: "#2a9d8f",
    borderWidth: 1.6,
    pointRadius: 0,
    tension: 0.25,
  });
  b.memba_hs.forEach((uh, k) =>
    ds.push({
  ...
  ds.push({
    label: "MANSAP (toplam)",
    data: y.hidrograflar[rp].map((v, i) => ({ x: i * dt, y: v })),
    borderColor: "#c73e3a",
    borderWidth: 2.2,
    pointRadius: 0,
    tension: 0.25,
  ...
  for (let i = 0; i < comb.length; i++) {
    ...
    rows.push([
      (i * dt).toFixed(1),
      ara === "" ? "" : (+ara).toFixed(2),
      ...mem.map((v) => v.toFixed(2)),
      comb[i].toFixed(2),
    ]);
    h +=
      `<tr><td>${fmt(i * dt, 1)}</td><td>${ara === "" ? "—" : fmt(ara, 2)}</td>` +
      mem.map((v) => `<td>${fmt(v, 2)}</td>`).join("") +
      `<td><b>${fmt(comb[i], 2)}</b></td></tr>`;
  }
  h += `</table>`;
...
function renderMcmpPik() {
  ...
  _mkChart(
    { labels, datasets: [{ label: `Q${rp} mansap piki`, data, backgroundColor: methods.map((m) => M_COLORS[m]) }] },
    `Mansap Q${rp} pik debileri (öteleme sonrası)`,
    "bar",
  );
  let h =
    `<h3 class="res">Mansap Pikleri — yöntem × tekerrür (m³/s)</h3>
    <table class="tbl"><tr><th>Yöntem</th>` +
    MRP.map((t) => `<th>Q${t}</th>`).join("") +
    `</tr>`;
  methods.forEach((m) => {
    const p = rt.yontemler[m].pikler || {};
    h +=
      `<tr><td style="border-left:4px solid ${M_COLORS[m]}">${M_LABEL[m]}</td>` +
      MRP.map((t) => `<td class="${t === rp ? "max" : ""}">${p[t] == null ? "—" : fmt(p[t], 1)}</td>`).join("") +
      `</tr>`;
  });
...
function renderMcmpHidro() {
  ...
  methods.forEach((m) => {
    const y = rt.yontemler[m];
    const arr = (y.hidrograflar || {})[rp];
    if (!arr) return;
    const dt = y.dt,
      pts = arr.map((v, i) => ({ x: i * dt, y: v }));
    ds.push({
      label: M_LABEL[m],
      data: pts,
  ...
  let h =
    `<h3 class="res">Koordinatlar (Q${rp})</h3><button id="btnMcmpCsv2" class="small-btn">⬇ CSV</button>
    <table class="tbl"><tr><th>T (sa)</th>` +
    series.map((s) => `<th>${M_LABEL[s.m]}</th>`).join("") +
    `</tr>`;
  const rows = [];
